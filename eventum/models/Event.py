...
class Event(Document, BaseEventumDocument):
    ...
    def get_absolute_url(self):
        ...
        if self.is_recurring:
            return url_for(current_app.config['EVENTUM_RECURRING_EVENT_ROUTE'],
                           slug=self.slug,
                           index=self.index)
        return url_for(current_app.config['EVENTUM_SINGLE_EVENT_ROUTE'],
                       slug=self.slug)

    @staticmethod
    def event_base_url():
        """Returns the first part of an event URL"""
        return url_for(current_app.config['EVENTUM_SINGLE_EVENT_ROUTE'],
                       slug='',
                       _external=True)
